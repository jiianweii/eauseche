...
export const useCreateProduct = async (product) => {
  ...
  const URL = "http://localhost:3000/api/v1/products/add";
...
export const useUpdateProduct = async (product, id) => {
  ...
  const URL = "http://localhost:3000/api/v1/products/update";
  const data = await fetch(URL, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
...
export const useDeleteProduct = async (id) => {
  const URL = "http://localhost:3000/api/v1/products/remove";
  const data = await fetch(URL, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
...
export const fetchProducts = async (gender, type, sort, page) => {
  ...
  const products = await fetch(
    `http://localhost:3000/api/v1/products${g}${q}${t}${x}${s}${px}${p}`
  );
...
export const useFetchProductById = async (gender, id) => {
  const URL = `http://localhost:3000/api/v1/products/${gender}/${id}`;
...
export const useFetchOrdersById = async (id) => {
  const token = localStorage.getItem("jwt");
  const URL = `http://localhost:3000/api/v1/orders/${id}`;
  const data = await fetch(URL, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
...
export const useCreateOrder = async (order) => {
  const token = localStorage.getItem("jwt");
  const URL = `http://localhost:3000/api/v1/orders/add`;
  const data = await fetch(URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
export const useUpdateStatus = async (id, status) => {
  const URL = `http://localhost:3000/api/v1/orders/order/${id}`;
  const data = await fetch(URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
